...
# ---------------------------------------------------
# 1. Load environment variables + create DB engine
# ---------------------------------------------------
...
with col2:
    st.image(r"\Assignment_Feedback_viewer\Assignment_Feedback_Viewer-1\gui_code\log.png", width=350)

# ---------------------------------------------------
# 2. SQL Queries (Written for each UI element)
# ---------------------------------------------------
...
def get_colleges():
    with engine.begin() as conn:
        return [row.college_name for row in conn.execute(Query_Colleges)]
...
student_id = students_df[students_df["student_name"] == selected_student]["student_id"].values[0]
...
if st.session_state.prev_assignment != selected_assignment:
    ...
    st.session_state.show_status = False
    st.session_state.view_logged = False
resource_id = assignments_df[assignments_df["resource_title"] == selected_assignment]["resource_id"].values[0]
...
if st.session_state.show_status:
    ...
    activity_time = datetime.now(pytz.timezone("Asia/Kolkata"))
...
if download_clicked:
    ...
    latest_feedback = (
        latest["feedback_comments"]
        if latest is not None and latest["feedback_comments"]
        ...
        with engine.begin() as conn:
            conn.execute(
                QUERY_INSERT_ACTIVITY,
                {
                    ...
                    "assignment_name": selected_assignment,
                    "assignment_status": status_text,
                    "action": "Downloaded",   # ✅ explicitly set
